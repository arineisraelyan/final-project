...
library(dplyr)
library(patchwork)
# p1 <- panik |> filter(temperature>0) |> drop_na() |> ggplot(aes(x=time, y=temperature)) + geom_line(color="darkblue") + geom_smooth(color="white") + theme_minimal() + labs(subtitle="Dangerous humidity levels were registered from the middle of June to the start of July", x=NULL, y="Temperatue", title="Temperature and Humidity in Panik") + scale_x_datetime(
#     limits = c(as.POSIXct("2024-05-27 00:00"), as.POSIXct("2024-07-12 00:00")),
#     date_labels = "%b %d"
#   )
# 
# p2 <- panik |> drop_na() |> ggplot(aes(x=time, y=humidity)) + geom_line(aes(color=if_else(humidity<30,"too_low", "normal")))+ scale_color_manual(values = c("too_low"="red", normal="orange"), guide="none") + theme_minimal() + labs(x="Time", y="Humidity") + scale_x_datetime(
#     limits = c(as.POSIXct("2024-05-27 00:00"), as.POSIXct("2024-07-12 00:00")),
#     date_labels = "%b %d"
#   )
# 
...
# print(combined_plot)
# 
# ggsave(combined_plot, filename="panik_first.png", limitsize = FALSE)
knitr::include_graphics("panik_first.png")
```
...
p <- panik |> drop_na() |> filter(temperature > 30) |> mutate(humidity_level = factor(if_else(humidity < 30, "Too low", "Normal"))) |> ggplot(aes(x = time, y = temperature, alpha = 0.5)) + geom_point(aes(color = humidity_level)) + scale_color_manual(values = c("Too low" = "firebrick1", "Normal" = "steelblue1")) + theme_bw() + labs(title="Humidity level in Panik", subtitle="Humidity levels are lower than normal", color = "Humidity level", x = "Time", y = "Temperature") + scale_alpha_continuous(guide = "none") + scale_x_datetime(
    limits = c(as.POSIXct("2024-05-27 00:00"), as.POSIXct("2024-07-12 00:00")),
    date_labels = "%b %d"
  )
...
```{r message=FALSE}
# windplot <- panik |> drop_na() |> ggplot(aes(x=time, y=speed)) + geom_jitter(aes(color=if_else(speed<15,"normal", "too_high"), alpha=0.5))+ scale_color_manual(values = c("normal"="black", too_high="darkred"), labels=c("normal"="Normal Speed", "too_high"="High Speed")) + scale_alpha_continuous(guide = "none") + geom_smooth(color="white") + theme_minimal() + labs(title="Wind speed in Panik", x ="Time", y="Speed", color="Wind speed") + scale_x_datetime(
#     limits = c(as.POSIXct("2024-05-27 00:00"), as.POSIXct("2024-07-12 00:00")),
#     date_labels = "%b %d"
#   )
# 
# ggsave(windplot, filename="panik_wind.png", limitsize = FALSE)
knitr::include_graphics("panik_wind.png")
```
...
# p_circular <- panik |> drop_na() |> mutate(direction = factor(direction, levels = order)) |>
...
#   ) + labs(title="Wind direction counts in Panik", subtitle="The wind often blows from East")
# 
# print(p_circular) 
# 
# ggsave(p_circular, filename="panik_direction.png", limitsize = FALSE)
knitr::include_graphics("panik_direction.png")

# start_date <- as.POSIXct('2024-05-27 00:00')
# end_date <- as.POSIXct('2024-07-12 00:00')
# 
# filtered_data <- panik |>
