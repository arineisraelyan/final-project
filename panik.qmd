...
```{r message=FALSE}
panik |> drop_na() |> ggplot(aes(x=time, y=speed)) + geom_jitter(aes(color=if_else(speed<15,"normal", "too_high"), alpha=0.5))+ scale_color_manual(values = c("normal"="black", too_high="darkred"), labels=c("normal"="Normal Speed", "too_high"="High Speed")) + scale_alpha_continuous(guide = "none") + geom_smooth(color="white") + theme_minimal() + labs(title="Wind speed in Panik", x ="Time", y="Speed", color="Wind speed") + scale_x_datetime(
    limits = c(as.POSIXct("2024-05-27 00:00"), as.POSIXct("2024-07-12 00:00")),
    date_labels = "%b %d"
  )
```
