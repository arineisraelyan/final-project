...
data |> mutate(humidity_danger=ifelse(humidity < 30 & temperature>25, temperature-humidity, 0), .before=1)
print(data)
...
plot <- data |> ggplot(mapping=aes(x=time, y=humidity_danger, group=device, color=device)) + geom_line() + geom_point() + ggtitle("Humidity danger level in different areas") + theme_ipsum() + labs(x="Time", y="Danger level") + transition_reveal(time)
